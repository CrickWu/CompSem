...
import os
#word2vec = tf.load_op_library('/usr2/home/wchang2/course/11727_project/word2gm/word2vec_ops.so')
word2vec = tf.load_op_library('/usr1/home/wchang2/course/11727_project/src/utils/word2vec_ops.so')
import re
import operator
#import sys
import pandas as pd
...
class Word2GM(object):
    ...
    def load_vocab(self):
        id2word = [''.join([i if ord(i) < 128
            ...
            for line in open(os.path.join(self.save_path, 'vocab.txt'), 'rt')
            ]
